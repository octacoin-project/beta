...
*Setup instructions for a gitian build of Octacoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Octacoin
...
- [Building Octacoin](#building-octacoin)
...
Clone the git repositories for octacoin and gitian and then checkout the octacoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the octacoin repository
...
To build Octacoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the octacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/octacoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/octacoin-project/octacoin
...
For example:
```bash
URL=https://github.com/user/octacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit octacoin=${COMMIT} --url octacoin=${URL} ../octacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit octacoin=${COMMIT} --url octacoin=${URL} ../octacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit octacoin=${COMMIT} --url octacoin=${URL} ../octacoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/octacoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/octacoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/octacoin-build.assert
```
...
[octacoin/gitian.sigs](https://github.com/octacoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
